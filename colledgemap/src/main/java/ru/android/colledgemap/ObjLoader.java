...
public class ObjLoader {
    ...
    //Функция загрузки файла трехмерной модели из ресурсной папки
    ...
    public void parseModel(String model)
    {
        ...
        for(int i = 0; i < VertexStore.size()-1; i++)
        {
        ...
        indx = 0;
        norms = new float[NormalStore.size()*3];
        for(int i = 0; i < NormalStore.size()-1; i++)
        {
            norms[indx++] = NormalStore.get(i).x;
            norms[indx++] = NormalStore.get(i).y;
            norms[indx++] = NormalStore.get(i).z;
        }
        ...
        normsBuffer = ByteBuffer.allocateDirect(norms.length*4).order(ByteOrder.nativeOrder()).asFloatBuffer();
        normsBuffer.put(norms).position(0);
    ...
    private void parseNormalStores(int i, String l, String[] norms){
        String[] n = l.split("[ ]");
        NormalStore.add(new Vector3(Float.parseFloat(n[1]), Float.parseFloat(n[2]), Float.parseFloat(n[3])));
